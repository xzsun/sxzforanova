...
#
#=================================================================#
# one-way ANOVA 和 事后显著性检验
# Copyright (c) by Mr. Sun Xinzhan
# Programming time: July. 1st, 2021
# The following: 模拟数据
#=================================================================#
#' Title
#'
...
#'
#' @examples
# df <- data.frame(tr=factor(c("A","A","A","B","B","B","C","C","C")),va=c(2,3,3,3,4,4,4,5,5))
sxzforanova <- function(df){
  ...
  if(trnum>2){ # ANOVA
    if(pvalue1>=0.05 & pvalue2>=0.05){#Duncan检验
      ...
      if(plg1>=0.05 & plg2>=0.05){#对数转化后Duncan检验
        ...
        out <- duncan.test(model1, 'tr', DFerror, MSerror, alpha = 0.05, group=TRUE, main = NULL,console=FALSE)
        outabc <- data.frame(out$groups)
      }else if(plg1<0.05 | plg2<0.05){#非参数KW秩和检验，并用fdr对p值矫正
  ...
  write.xlsx(list_data,paste("df",date,".xlsx",sep=""),row.names=FALSE)
  print(data1)
